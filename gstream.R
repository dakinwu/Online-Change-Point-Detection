...
getZL = function(distM, k = 1){
  L = dim(distM)[1]
  A = matrix(0,L,k) # 用於存儲每個點的前 k 個最近鄰居的索引
  for (i in 1:L){
    A[i,] = (sort(distM[i,1:L], index.return=T)$ix)[1:k] # 找前 k 個近的
  }
  temp = table(A) # 包含矩陣 A 中每個索引出現的次數
  id = as.numeric(row.names(temp))
  deg = rep(0,L)
  deg[id] = temp # 根據 temp 中的次數更新 deg，這代表每個點的degree（即與其他點的連接數）
  deg.sumsq = sum(deg^2)
  cn = sum((deg-k)^2)/L/k # degree的變異數與預期degree（K）的差異。
  count = 0
  for (i in 1:L){
    ids = A[i,]
    count = count + length(which(A[ids,]==i)) # 對於每個點，計算與該點相連的點中有多少是互相連接的
  }
  vn = count/L/k # 平均每個點與其 k 個最近鄰居的互連數
...
getscanZ = function(distM,N0,L,N,k,n0,n1,statistics="all"){
  ...
  maxZloc = maxZwloc = maxSloc = maxMloc= rep((N0+1):N)
  for (n in (N0+1):N){
    #n0s = n-L+n1
    #n1s = n-n0
    tests = getZL(distM[(n-L+1):n,(n-L+1):n],k)
    maxZ[n-N0] = max(tests$Z[n0:n1])
    maxZloc[n-N0] = which.max(tests$Z[n0:n1])+n-n1-1
    maxZw[n-N0] = max(tests$Zw[n0:n1])
    maxZwloc[n-N0] = which.max(tests$Zw[n0:n1])+n-n1-1
    maxS[n-N0] = max(tests$S[n0:n1])
    maxSloc[n-N0] = which.max(tests$S[n0:n1])+n-n1-1
    maxM[n-N0] = max(tests$M[n0:n1])
    maxMloc[n-N0] = which.max(tests$M[n0:n1])+n-n1-1
  }
  
  scanZ = list()
  if (length(which(!is.na(match(c("o","ori","original","all"),statistics))))>0){
    scanZ$ori = maxZ
    scanZ$ori.loc = maxZloc
  }
  if (length(which(!is.na(match(c("w","weighted","all"),statistics))))>0){
    scanZ$weighted = maxZw
    scanZ$weighted.loc = maxZwloc
  }
  if (length(which(!is.na(match(c("m","max","g","generalized","all"),statistics))))>0){
    scanZ$max.type = maxM
    scanZ$max.type.loc = maxMloc
  }
  if (length(which(!is.na(match(c("g","generalized","all"),statistics))))>0){
    scanZ$generalized = maxS
    scanZ$generalized.loc = maxSloc
  }
  return(scanZ)
}
