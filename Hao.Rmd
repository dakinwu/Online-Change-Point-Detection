...
```{r}
check_criteria <- function(location_values, delay_values) {
  # 找到連續三個值都在72到78之間的序列
  valid_locs <- which(location_values >= 72 & location_values <= 78)
  for (idx in valid_locs) {
    if (idx > (length(location_values) - 2)) {
      break
    }
    if (all(location_values[idx:(idx+2)] >= 72 & location_values[idx:(idx+2)] <= 78) &&
        all(delay_values[idx:(idx+2)] > 0 & delay_values[idx:(idx+2)] <= 15)) {
      return(TRUE)
    }
  }
}

scoring <- function(score_list = score, resultmat, i, scorevec, location, delay) {
  if(i == 1) {
    scorevec[[location]] <- 0
  }
  location_values <- unlist(resultmat[[i]][[location]])
  delay_values <- unlist(resultmat[[i]][[delay]])
  
  if(check_criteria(location_values, delay_values)) {
    scorevec[[location]] <- scorevec[[location]] + 1
    score_list[[location]][i] = 1
  }
  return(list(scorevec, score_list))
}

scoring <- function(delay_list, resultmat, i, scorevec, location, delay, alert, alert_list) {
  if(i == 1) {
    scorevec[[location]] <- 0
  }
  alert_values <- unlist(resultmat[[i]][[alert]])
  alert_list[[location]][i] = alert_values[1]
  delay_values <- unlist(resultmat[[i]][[delay]])
  if(!is.na(delay_values[1]) && delay_values[1] > 0) {
    scorevec[[location]] <- scorevec[[location]] + 1
    delay_list[[location]][i] = delay_values[1]
  }
  return(list(scorevec, delay_list, alert_list))
}

performance <- function(results, resultsa) {
  delays <- readRDS(results); alerts <- readRDS(resultsa)
  cbind(lapply(delays, function(x) sum(!is.na(x))), lapply(delays, function(x) round(mean(x, na.rm = TRUE), 3)), lapply(delays, function(x) 100-length(x)))
}

# what's causing the error?
# (25, 39, 15, 35, 5, 3, 1, 9.6, 0.3733333, 3.2933333, 0.2533333, 3.2666667, 0.2133333, 3.0266667, 945, 4797, 159, 205, 517, 2869)
# T3.skewed.lambdaM(25, 39, 15, 35, 5, 3, 1, 9.6, 0.3733333, 3.2933333, 0.2533333, 3.2666667, 0.2133333, 3.0266667, 945, 4797, 159, 205, 517, 2869)
# T3.skewed.lambdaZd(39, 15, 35, 5, 3, 1, 9.6, 0.3733333, 3.2933333, 0.2533333, 3.2666667, 0.2133333, 3.0266667, 945, 4797, 159, 205, 517, 2869)
...
start = Sys.time()
N = 800
testing <- function(N0 = 75, L = 50, k = 3, z_arl, zw_arl, s_arl){ # , m_arl
  distance_matrix = sim1(rho=0.2, mean_change=FALSE, scale=1, cov_change = FALSE)$distance_matrix
  r1 = gstream(distance_matrix, L, N0, k, statistics=c("o","w","g"), n0=0.3*L, n1=0.7*L, ARL=500, alpha=0.05, skew.corr=TRUE, asymp=FALSE) # "all"
  z_arl = c(z_arl, r1$tauhat$ori[1]+N0)
  zw_arl = c(zw_arl, r1$tauhat$weighted[1]+N0)
  # m_arl = c(m_arl, r1$tauhat$max.type[1]+N0)
  s_arl = c(s_arl, r1$tauhat$generalized[1]+N0)
  return(list(z_arl = z_arl, zw_arl = zw_arl, s_arl = s_arl)) # , m_arl = m_arl
}
i = 1; z_arl = c(); zw_arl = c(); s_arl = c() #  m_arl = c();
while(length(z_arl) < 50){
  tryCatch({
    print(paste0("Run: ", i))
    results = testing(75, 50, 3, z_arl, zw_arl, s_arl) # , m_arl
    z_arl = results$z_arl; zw_arl = results$zw_arl; s_arl = results$s_arl # ; m_arl = results$m_arl
    i = i + 1
  }, error = function(e) {
...
print(paste0("took: ", round(as.numeric(difftime(time1 = end, time2 = start, units = "secs")), 3), " seconds"))
mean(z_arl); mean(zw_arl); mean(s_arl) # ; mean(m_arl)
```
...
#   print("----------------------------------")
# }
saveRDS(alerts, "gstream_eucli_mean_1500arl_alerts_cp100.rds")
saveRDS(delays, "gstream_eucli_mean_1500arl_delays_cp100.rds")

setwd("C:/Users/dakin/Desktop/Online CPD/multivariate data online CPD/graphing/GStream/gstream_eucli_rho0.2")
performance("gstream_eucli_mean_500arl_delays_cp150.rds", "gstream_eucli_mean_500arl_alerts_cp150.rds")
performance("gstream_eucli_mean_1000arl_delays_cp150.rds", "gstream_eucli_mean_1000arl_alerts_cp150.rds")
performance("gstream_eucli_mean_1500arl_delays_cp150.rds", "gstream_eucli_mean_1500arl_alerts_cp150.rds")
performance("gstream_eucli_mean_500arl_delays_cp100.rds", "gstream_eucli_mean_500arl_alerts_cp100.rds")
performance("gstream_eucli_mean_1000arl_delays_cp100.rds", "gstream_eucli_mean_1000arl_alerts_cp100.rds")
performance("gstream_eucli_mean_1500arl_delays_cp100.rds", "gstream_eucli_mean_1500arl_alerts_cp100.rds")
```
...
saveRDS(alerts, "gstream_eucli_scale_1500arl_alerts_cp100.rds")
saveRDS(delays, "gstream_eucli_scale_1500arl_delays_cp100.rds")

performance("gstream_eucli_scale_500arl_delays_cp150.rds", "gstream_eucli_scale_500arl_alerts_cp150.rds")
performance("gstream_eucli_scale_1000arl_delays_cp150.rds", "gstream_eucli_scale_1000arl_alerts_cp150.rds")
performance("gstream_eucli_scale_1500arl_delays_cp150.rds", "gstream_eucli_scale_1500arl_alerts_cp150.rds")
performance("gstream_eucli_scale_500arl_delays_cp100.rds", "gstream_eucli_scale_500arl_alerts_cp100.rds")
performance("gstream_eucli_scale_1000arl_delays_cp100.rds", "gstream_eucli_scale_1000arl_alerts_cp100.rds")
performance("gstream_eucli_scale_1500arl_delays_cp100.rds", "gstream_eucli_scale_1500arl_alerts_cp100.rds")
```
...
saveRDS(alerts, "gstream_eucli_mean_scale_1500arl_alerts_cp100.rds")
saveRDS(delays, "gstream_eucli_mean_scale_1500arl_delays_cp100.rds")

performance("gstream_eucli_mean_scale_500arl_delays_cp150.rds", "gstream_eucli_mean_scale_500arl_alerts_cp150.rds")
performance("gstream_eucli_mean_scale_1000arl_delays_cp150.rds", "gstream_eucli_mean_scale_1000arl_alerts_cp150.rds")
performance("gstream_eucli_mean_scale_1500arl_delays_cp150.rds", "gstream_eucli_mean_scale_1500arl_alerts_cp150.rds")
performance("gstream_eucli_mean_scale_500arl_delays_cp100.rds", "gstream_eucli_mean_scale_500arl_alerts_cp100.rds")
performance("gstream_eucli_mean_scale_1000arl_delays_cp100.rds", "gstream_eucli_mean_scale_1000arl_alerts_cp100.rds")
performance("gstream_eucli_mean_scale_1500arl_delays_cp100.rds", "gstream_eucli_mean_scale_1500arl_alerts_cp100.rds")
```
...
saveRDS(alerts, "gstream_eucli_cov_1500arl_alerts_cp100.rds")
saveRDS(delays, "gstream_eucli_cov_1500arl_delays_cp100.rds")

performance("gstream_eucli_cov_500arl_delays_cp150.rds", "gstream_eucli_cov_500arl_alerts_cp150.rds")
performance("gstream_eucli_cov_1000arl_delays_cp150.rds", "gstream_eucli_cov_1000arl_alerts_cp150.rds")
performance("gstream_eucli_cov_1500arl_delays_cp150.rds", "gstream_eucli_cov_1500arl_alerts_cp150.rds")
performance("gstream_eucli_cov_500arl_delays_cp100.rds", "gstream_eucli_cov_500arl_alerts_cp100.rds")
performance("gstream_eucli_cov_1000arl_delays_cp100.rds", "gstream_eucli_cov_1000arl_alerts_cp100.rds")
performance("gstream_eucli_cov_1500arl_delays_cp100.rds", "gstream_eucli_cov_1500arl_alerts_cp100.rds")
```
...
simf1<-function(t){-2*sin(t-1)*log(t+0.5)}
simf2<-function(t){2*cos(t)*log(t+0.5)}
simf3<-function(t){-0.5-0.2*cos(0.5*(t-1))*t^(1.5)*sqrt(5*sqrt(t)+0.5)}
simf4<-function(t){1.2*cos(t)*log(t+0.5)*sqrt(t+0.5)}
#sigmaf<-function(t){rep(sqrt(.5), length(t))}
...
heter=F
# rho 時間序列相關係數
m=200; n.curve=100; rho=0.5; nbasis=15
Cor.mat<-rho^abs(outer((1:m), (1:m), '-'));
svdcor=svd(Cor.mat)
...
E2=Cor.math%*%matrix(rnorm(m*n.curve, sd=sqrt(.5)), m, n.curve)
y=matrix(rnorm(m*n.curve), m, n.curve)*sigmaf(t)+E2
y[,1:25]=y[, 1:25]+simf1(t)
y[,26:50]=y[, 26:50]+simf2(t)
y[,51:75]=y[, 51:75]+simf3(t)
y[,76:n.curve]=y[, 76:n.curve]+simf4(t)
...
# testing on simulated y
library(fda)
heter = T; n.curve = 100; rho = 0.2; nbasis = 150
# 定義傅立葉基函數對象
fourierBasis <- create.fourier.basis(rangeval=range(t_values), nbasis=L+1)
# 將模擬數據轉換為函數數據對象
Y_simulated = t(Y_simulated)
Y_fd <- Data2fd(argvals=t_values, y=Y_simulated, basisobj=fourierBasis)
t.grid <- t_values
B.grid <- eval.basis(t.grid, fourierBasis)
# 計算基函數的懲罰矩陣
P <- getbasispenalty(fourierBasis)
```
...
if(heter == F){ 
  ...
  print('Heterogeneous error')
  for (i in 1:n.curve){
      # 對於異質的誤差，先對y進行擬合以估計誤差分佈，然後利用估計結果作為權重來重新擬合模型，從而調整誤差的異質性
...
# testing on simulated y
B = B.grid; y = Y_simulated
arrayVp=array(NA, c(nbasis+1, nbasis+1, n.curve))
coef=matrix(NA, nbasis+1, n.curve)
...
fit=weight.minCV(coef=coef, arrayVp=arrayVp, B.grid=B.grid, B.grid.weight=B.grid.w, t.grid=t.grid)
weight=fit$weight
plot(t.grid, weight, type='l')
```